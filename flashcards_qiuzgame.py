...
if __name__ == "__main__":
    try:
        ...
        manager.run_menu("🎮 HỆ THỐNG HỌC TẬP FLASHCARD", {
            ...
            ("3" if is_num else "/mistakes"): (lambda: manager.show_mistake_stats(), "❌ Lỗi sai"),
            ...
            ("6" if is_num else "/clear_history"): (lambda: manager.clear_history(), "🧹 Xoá lịch sử"),
            ("7" if is_num else "/clear_log"): (lambda: manager.clear_logs(), "📝 Dọn sạch nhật ký"),
            ("8" if is_num else "/clear_trash"): (lambda: manager.empty_trash(), f"🗑️ Dọn sạch thùng rác"),
